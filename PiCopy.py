...
class PiCopy(tk.Frame):
    ...
    def init_display(self):
        ...
        self.display_storage_selection()
        ...
        self.reset_btn = tk.Button(self.frame, text='reset', command=lambda: self.reset_options(), bd=3, relief=tk.RAISED)
        self.submit_btn = tk.Button(self.frame, text='confirm selection', command=lambda: self.display_confirmation(), bd=3, relief=tk.RAISED)
        self.quit_btn = tk.Button(
        self.frame, text='quit', command=self.frame.quit, bd=3,relief=tk.RAISED)
        self.quit_btn.grid(row=1, column=1, ipadx=15, ipady=15, padx=10, sticky=tk.SE)
        self.populate_drive_list()

    def display_storage_selection(self):
    ...
    def populate_drive_list(self):
        ...
        for mounted_name in (next(os.walk("/media/pi"))[1]):
            ...
            source_storage = tk.Button(self.source_frame, text=display_name, command=self.create_callback_lambda( mounted_name, 'source'))
            dest_storage = tk.Button(self.dest_frame, text=display_name, command=self.create_callback_lambda(mounted_name, 'dest'))
    ...
    def create_callback_lambda(self, mounted_name, locale):
        return lambda: self.button_select(mounted_name, locale)

    def button_select(self, mounted_name, locale):
        reverse_locale_name = ' '
        for storage_drive in self.storage_drives:
        ...
        self.check_values_selected()

    def check_values_selected(self):
    ...
    def _begin_copy(self):
        ...
        source_size = self.measure_file(source_dir)
        ...
        copy_thread_class = CopyThread(
            source_dir, self.dest_dir, file_copy_list)
        copy_thread_class.copy_controller()
    ...
    def measure_file(self, file_dir):
        total_size = 0
...
class CopyThread():
    ...
    def copy_controller(self):
        ...
        def copy_thread(self, x, source_dir, dest_dir, file_name):
            self.sema.acquire()
            print('starti :' + str(x))
